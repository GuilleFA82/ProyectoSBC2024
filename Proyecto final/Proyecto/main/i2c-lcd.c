...
void lcd_send_float(float numero, int precision) {
    char buffer[20]; // Buffer para almacenar el número convertido a cadena

    // Convertir el float a cadena con la precisión especificada
    sprintf(buffer, "%.*f", precision, numero);

    // Enviar la cadena resultante a la LCD
